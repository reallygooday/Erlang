...
def planetchoice
    ...
    puts "Do you decide to land on planet Erlang, save Saturn, save Mars or do you remain on planet Earth?"
    ...
    if choice.include?("Erlang") || choice.include?("erlang")
        ...
        saturn
        elsif choice.include?("Mars") || choice.include?("mars")
        mars
        else
...
def mars
    puts "Welcome to Extraterrestrial Life, Martian. Mars is the most Earth-Like of all the planets. Russia is planning to colonise Mars. You must act fast !"
    sleep 2 #seconds
    puts "We understand that you had a hard time entering space and landing on Mars. "
    sleep 2 #seconds
    puts "We really proud of what you've found - a Tesla All-Purpose Rover - to move accross the Red Planet surface."
    sleep 2 #seconds
    puts "Stay focused, Martian! Crater sand buries you while you are getting distracted. "
    sleep 2 #seconds
    puts " Okay. Enough chit-chat, Martian. Let's see you in action."
    sleep 2 #seconds
    puts "You'll be presented with two questions at once. We know, Martian, this is a poor timing, but you must answer immediately."
    sleep 2 #seconds
    puts "Is there a water on Mars ?" 
    sleep 2 #seconds
    puts "Is a day on Mars is longer than one on Earth ? [yes/no]"
    ...
    if answermars.include?("yes")
        ...
        dead ("That is not exactly true. You did not know that water on Mars is frozen in the ground, and a day on Mars is longer than one on Earth. It is time to die, Martian.")
        else
        planetchoice
    end
end
